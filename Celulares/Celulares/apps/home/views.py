...
from Celulares.apps.ventas.models import Celular
from Celulares.apps.home.forms import contact_form
from django.core.mail import EmailMultiAlternatives
from Celulares.apps.home.forms import add_product_form, login_form
from django.contrib.auth import login, logout, authenticate
from django.http import HttpResponseRedirect
from django.core.paginator import Paginator, EmptyPage, InvalidPage

def pagprincipal_view(request):
	return render_to_response('home/pagprincipal.html', context_instance = RequestContext(request))
...
def celulares_view(request, pagina):
	lista_prod = Celular.objects.filter()
	...
	ctx  = {'celulares':productos}
	return render_to_response('home/celulares.html', ctx, context_instance =RequestContext(request))
...
def contacto_view(request):
	...
	if request.method == "POST":
		...
		if formulario.is_valid():
			...
			to_admin = 'ortizandres@misena.edu.co'
...
def single_celular_view(request, id_celular):
	prod = Celular.objects.get(id = id_celular)
	ctx  = {'celular':prod}
	return render_to_response('home/single_celular.html',ctx,context_instance = RequestContext(request))
