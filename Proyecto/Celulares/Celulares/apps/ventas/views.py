...
from Celulares.apps.home.forms import add_product_form
...
def add_product_view(request):
	...
	if request.method == "POST":
		...
		if formulario.is_valid():
			...
			return HttpResponseRedirect('/celular/%s'%add.id)
	else:
...
def edit_producto_view(request, id_prod):
	info = ""
	prod = Celular.objects.get(pk = id_prod)
	...
	if request.method == "POST":
		...
		if formulario.is_valid():
			...
			edit_prod.save()
			info = "se guardo satisfactoriamente"				
			return HttpResponseRedirect('/celular/%s'%edit_prod.id)
	else:
